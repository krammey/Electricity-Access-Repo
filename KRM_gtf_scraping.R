...
for(k in 1:length(COUNTRIES)){ # looping through countries to download each report
    ...
    txt2 <- strsplit(txt, "\n")
    pages <- length(txt2)
    for(i in 1:2){ # loop through first 2 pages of report
      ...
      y <- data.frame(y,stringsAsFactors=FALSE) # append the year to the end of the y dataframe
      # Capitalize country name
      country <- gsub("-"," ",country)
      y$country <- sapply(country, simpleCap) # this appends each y dataframe (an individual PDFs worth of lines) onto x
      x <- rbind(x, y)
    } # end i
} # end k
